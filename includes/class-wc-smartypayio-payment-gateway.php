...
class WC_SmartyPayIo_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function admin_options() {
        if ( in_array( get_woocommerce_currency(), $this->available_currencies ) ) {
            ...
            <div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'smartypayio-payment-gateway' ); ?></strong> <?php echo sprintf( __( 'Choose USDT, BUSD, USDT, USDC or MNXe as your store currency in %1$sGeneral Settings%2$s to enable the SMARTy Pay Gateway.', 'smartypayio-payment-gateway' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=general' ) ) . '">', '</a>' ); ?></p></div>
            <?php
        }
    }
